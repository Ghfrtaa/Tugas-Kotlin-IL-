...
class Hero {
    ...
    fun jalan(banyaknya : Int? = null) {
        checkHealth()
        println("$name sedang berjalan...")
        health -= 1 * (banyaknya ?: 1)
    }
    ...
    */
    ...
    fun lari(banyaknya: Int? = null) {
        checkHealth()
        println("$name sedang lari...")
        health -= 2 * (banyaknya ?: 1)
    }
    fun makan(banyaknya: Int? = null) {
        checkHealth()
        println("$name sedang makan...")
        health += 2 * (banyaknya ?: 1)
    }
    fun minum(banyaknya: Int? = null){
        checkHealth()
        println("$name sedang minum...")
        health += 2 * (banyaknya ?: 1)
    }
    fun lompat(banyaknya: Int? = null) {
        checkHealth()
        println("$name sedang lompat...")
        health -= 2 * (banyaknya ?: 1)
    }
    fun duduk(banyaknya: Int? = null){
        checkHealth()
